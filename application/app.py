...
"""
conn = psycopg2.connect(dbname=cfg.DBNAME, user=cfg.DBUSER, password=cfg.DBPWD, host=cfg.DBHOST, port=cfg.DBPORT)
curseur = conn.cursor()
...
def insert(table, colonnes, valeurs):
    query = "INSERT INTO %s(%s) VALUES(%s);" % (table, ",".join(colonnes), ",".join(["%s"] * len(colonnes)))
...
def ajouter_vehicule():
    ...
    insert("Vehicule", ["immat", "modele", "carburant", "couleur", "nb_km", "agence", "agent_tech"], valeurs)
"""
    Afficher des tables
"""
